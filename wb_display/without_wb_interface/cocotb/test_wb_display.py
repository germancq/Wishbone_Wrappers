...
async def test_send_data(dut, data_test, wb_master):
    ADDR_DISPLAY_DATA = dut.bridge_impl.DISPLAY_DATA.value

    await wb_master.setup_write_operation(ADDR_DISPLAY_DATA, data_test)

    check_state(dut.bridge_impl, dut.bridge_impl.DISPLAY_DATA_OP_0.value)

    await n_cycles_clk(dut,1)

    #comprobar que el dato esta en el display
    assert (dut.display_impl.din.value==data_test),f"ERROR SEND DATA, expected={hex(data_test)} calculated={hex(dut.display_impl.din.value)}"
    ...
    check_state(dut.bridge_impl, dut.bridge_impl.IDLE.value)

    assert (dut.display_impl.din.value==data_test),f"ERROR POST-SEND DATA, expected={hex(data_test)} calculated={hex(dut.display_impl.din.value)}"
